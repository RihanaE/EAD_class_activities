...
public class detailSearchServlet extends HttpServlet {
	...
	private static final String query = "select * from users where name like ? or accountNumber like ?";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List <Client> searchedList = new ArrayList<>();
		...
		try {
			...
			while(rs.next()) {
				...
				searchedList.add(detail);
			}
			session.setAttribute("searchedList",  searchedList);
			response.sendRedirect("detailSearch.jsp");
